...
import getMonsters from "./getwikidata/getMonsters";
import getWikiPages from "./getwikidata/getWikiPages";
import getWikiPage from "./getwikidata/getWikiPage";
import getMonster from "./getwikidata/getMonster";
import { processBatches, toStorable } from "./utils/misc";
...
export const getWikiData = onSchedule(
    ...
    async () => {
      ...
      const index: { name: string, tags: string[] }[] = [];
      ...
      const cleanedPages = allPages
          .filter((i): i is { name: string; tags: string[]; html: string } => i?.html != null)
          .map((i) => ({ ...i, html: i.html }));

      const batches = await processBatches(cleanedPages, 500);
