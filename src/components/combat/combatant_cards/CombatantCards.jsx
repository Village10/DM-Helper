...
export default function CombatantCards({ combatants, setCombatants, selected, setSelected, setOpenEdit, turn }) {
	return (
		<Grid
			container
			spacing={2}
					...
					>
						<Card
							sx={(theme) => ({
								...
								boxShadow: index === (turn - 1) % combatants.length ? `5px 5px 20px -5px ${theme.palette.secondary.main}` : 'none' })}
						>
							<Box
								sx={{
									position: 'absolute',
								...
								>
									...
									>
										...
										>
											<LocalHospital/>
											<Typography>
												{Math.min(combatant.health, 999) + ' / ' + combatant.max_health}
											</Typography>
										</Stack>
									</Tooltip>
									<Tooltip
